...
// Carregar lista de cargo ao inicializar
...
async function buscarCargo() {
    ...
    try {
        ...
        if (response.ok) {
            ...
            mostrarMensagem('Cargo não encontrado. Você pode incluir um nova cargo.', 'info');
...
// Função para carregar lista de cargo
async function carregarcargo() {
    ...
    //    debugger
        if (response.ok) {
            const cargo = await response.json();
            renderizarTabelacargo(cargo);
        } else {
            throw new Error('Erro ao carregar cargo');
        }
    } catch (error) {
        console.error('Erro:', error);
        mostrarMensagem('Erro ao carregar lista de cargo', 'error');
    }
}

// Função para renderizar tabela de cargo
function renderizarTabelacargo(cargo) {

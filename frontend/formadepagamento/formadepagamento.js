...
async function buscarFormadepagamento() {
    ...
    try {
        ...
        if (response.ok) {
            ...
            mostrarMensagem('Forma de pagamento encontrado!', 'success');
            ...
            mostrarMensagem('Forma de pagamento não encontrado. Você pode incluir uma nova forma de pagamento.', 'info');
...
async function excluirFormadepagamento() {
    mostrarMensagem('Excluindo forma de pagamento...', 'info');
    currentPersonId = searchId.value;
    //bloquear searchId
...
async function salvarOperacao() {
    ...
    try {
        ...
        if (response.ok && (operacao === 'incluir' || operacao === 'alterar')) {
            ...
            mostrarMensagem(error.error || 'Erro ao incluir forma de pagamento', 'error');
        } else {
        ...
        console.error('Erro:', error);
        mostrarMensagem('Erro ao incluir ou alterar a forma de pagamento', 'error');
    }
    ...
    try {
     ...
     //   console.log(JSON.stringify(response));
        ...
        if (response.ok) {
            ...
            throw new Error('Erro ao carregar forma de pagamentos');
        }
    } catch (error) {
        console.error('Erro:', error);
        mostrarMensagem('Erro ao carregar lista de forma de pagamentos', 'error');
    }
}

...
// Carregar lista de produto ao inicializar
...
async function buscarProduto() {
    ...
    try {
        ...
        if (response.ok) {
            ...
            mostrarMensagem('Produto não encontrado. Você pode incluir um nova produto.', 'info');
...
async function carregarproduto() {
    ...
    //    debugger
        if (response.ok) {
            const produto = await response.json();
            renderizarTabelaproduto(produto);
        } else {
            throw new Error('Erro ao carregar produto');
        }
    } catch (error) {
        console.error('Erro:', error);
        mostrarMensagem('Erro ao carregar lista de produto', 'error');
    }
}

// Função para renderizar tabela de produto
function renderizarTabelaproduto(produto) {

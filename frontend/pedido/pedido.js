...
async function buscarPedido() {
    ...
    try {
        ...
        if (response.ok) {
            ...
            await carregarItensDoPedido(pedido.idpedido);
...
function preencherFormulario(pedido) {
    ...
    //  console.log('data pedido: ' + pedido.datadopedido);
    //  console.log('data pedido formatar: ' + formatarDataParaInputDate(pedido.datadopedido));

    currentPersonId = pedido.idpedido;
    searchId.value = pedido.idpedido;
    document.getElementById('datadopedido').value = formatarDataParaInputDate(pedido.datadopedido);

    document.getElementById('clientepessoacdpessoa').value = pedido.clientepessoacdpessoa || 0;
    document.getElementById('funcionariopessoacdpessoa').value = pedido.funcionariopessoacdpessoa || 0;
...
async function salvarOperacao() {
    ...
    const pedido = {
        ...
        clientepessoacdpessoa: formData.get('clientepessoacdpessoa'),
        funcionariopessoacdpessoa: formData.get('funcionariopessoacdpessoa'),
    };
    let response = null;
    try {
        if (operacao === 'incluir') {
...
function renderizerTabelaItensPedido(itens) {
    ...
    itens.forEach((item, index) => {
        ...
        row.innerHTML = `
        <td>${item.pedidoidpedido}</td>                  
        <td>${item.produtoidproduto}</td>
...
function renderizarTabelaPedidos(pedidos) {
    ...
    pedidos.forEach(pedido => {
        ...
        row.innerHTML = `
                    ...
                    <td>${pedido.clientepessoacdpessoa}</td>                  
                    <td>${pedido.funcionariopessoacdpessoa}</td>                  
...
async function buscarProdutoPorId(button) {
    ...
    try {
        ...
        const precoInput = row.querySelector('.preco-input');
        precoInput.value = produto.precounitario;

        const nomeprodutoInput = row.querySelector('td:nth-child(3) span');
        nomeprodutoInput.innerHTML = produto.nomeproduto;

        // Atualiza o subtotal da linha
        atualizarSubtotal({ target: precoInput });

        mostrarMensagem(`Produto ${produto.nomeproduto} encontrado!`, 'success');
...
function btnAdicionarItem(button) {
    ...
    const itemData = {
        pedidoidpedido: parseInt(pedidoId),
    ...
    if (isNaN(itemData.pedidoidpedido) || isNaN(itemData.produtoidproduto) || isNaN(itemData.quantidade) || isNaN(itemData.precounitario)) {
...
function btnAtualizarItem(button) {
    ...
    const itemData = {
        pedidoidpedido: parseInt(pedidoId),
        produtoidproduto: parseInt(produtoId),
        nomeproduto: nomeProduto.replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/\n/g, " "), // Sanitiza e remove quebras de linha  
        quantidade: parseInt(quantidade),
        precounitario: parseFloat(precoUnitario.replace(',', '.'))
    };
    ...
    delete itemData.nomeproduto;

    //alert("Dados do item a serem salvos:" + JSON.stringify(itemData));

    // 6. Envia os dados para o backend usando fetch.
    // Ajuste a URL e o método conforme sua API. router.put('/:idpedido/:id_produto', pedidohasprodutoController.atualizarpedidohasproduto);
    // Note que estamos enviando tanto o id do pedido quanto o id do produto na URL.
    fetch(`${API_BASE_URL}/pedidohasproduto/${itemData.pedidoidpedido}/${itemData.produtoidproduto}`, {
